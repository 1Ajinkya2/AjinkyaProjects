...
struct AVLNode *searchdefinition(struct AVLNode *node, char *definition)
{
    if (node == NULL || strcmp(node->definition, definition) == 0)
    {
        return node;
    }
    if (strcmp(definition, node->definition) < 0)
    {
        return searchdefinition(node->left, definition);
    }
    else
    {
        return searchdefinition(node->right, definition);
    }
}


struct AVLNode *insertree(struct AVLNode *node, char *word, char *definition)
{
    if (node == NULL)
    {
        struct AVLNode *new_node = newNode(word, definition);
        ...
        fprintf(fp, "%s:%s", word, definition);
        fclose(fp);
        return new_node;
    }
...
struct AVLNode *searchdefinition(struct AVLNode *node, char *definition)
{
    ...
    if (strcmp(definition, node->definition) < 0)
    {
        return search(node->left, definition);
    }
    else
    {
        return search(node->right, definition);
    }
}
...
int main()
{
    ...
    while (1)
    {
        ...
        case 1:
        {
            ...
            root = insertree(root, word, definition);
